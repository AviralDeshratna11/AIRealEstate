          ...
          <div className="sticky top-4 space-y-4 rounded-xl border border-ink/12 bg-ivory p-4 shadow-lx">
            ...
            <nav className="space-y-1">
              {navItems.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className={clsx("flex items-center gap-3 rounded-xl px-3 py-3 transition", selected ? "bg-ink text-ivory" : "text-ink/60 hover:bg-sand")}>
        ...
        <section className="min-w-0 flex-1 space-y-5 pb-8">
          <header className="portal-surface rounded-xl border border-ink/12 px-5 py-5 shadow-lx">
            ...
            <div className="flex flex-wrap gap-3">
                <Link href="/" className="inline-flex items-center gap-2 rounded-xl border border-ink/15 bg-ivory px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ink transition-colors hover:bg-sand">
                  <Home size={16} />
                  Home
                </Link>
                <Link href="/workspace" className="inline-flex items-center gap-2 rounded-xl border border-ink/15 bg-ivory px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ink transition-colors hover:bg-sand">
                  <Home size={16} />
                  Buyer OS
                </Link>
                <Link href="/manager" className="inline-flex items-center gap-2 rounded-xl border border-ink/15 bg-ivory px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ink transition-colors hover:bg-sand">
                  <BriefcaseBusiness size={16} />
                  Manager OS
                </Link>
                <Link href="/radar" className="inline-flex items-center gap-2 rounded-xl border border-gold/40 bg-gold/10 px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ink transition-colors hover:bg-gold hover:text-ivory">
                  <BriefcaseBusiness size={16} />
                  Broker Radar
                </Link>
                <button onClick={() => onRunAutomation(false)} className="inline-flex items-center gap-2 rounded-xl bg-gold px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1d4ed8]">
                  <Wand2 size={16} />
    ...
    <div className="grid gap-5 xl:grid-cols-[0.85fr_1.15fr]">
      <Panel title={mode === "onboarding" ? "Broker onboarding" : "Broker profile"} eyebrow="Verification agent">
        <div className="space-y-4">
          <div className="rounded-xl bg-espresso p-5 text-ivory">
          ...
          <button onClick={onSave} className="inline-flex items-center gap-2 rounded-xl bg-gold px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1d4ed8]">
            <BadgeCheck size={16} />
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
      ...
      <Panel title="AI shareable pitch" eyebrow="Verified-claims only">
        <p className="rounded-xl bg-espresso p-4 text-sm font-semibold leading-7 text-ivory">{property.shareable_pitch || `${property.title} in ${property.locality}. Request tie-up before sharing full details.`}</p>
    ...
    <Panel title="Tie-up request feed" eyebrow="Formal broker-manager agreements">
      <div className={clsx("grid gap-3", !compact && "xl:grid-cols-2")}>
        {tieups.length ? tieups.map((tieup) => {
          ...
          return <div key={tieup.id} className="rounded-xl border border-ink/12 bg-ivory p-4">
...
function PropertyPoolView({ events, onNew }: { events: PropertyPoolEvent[]; onNew: () => void }) {
  return (
    <div className="space-y-5">
      <div className="flex justify-end"><button onClick={onNew} className="inline-flex items-center gap-2 rounded-xl bg-ink px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1f2937]"><Plus size={16} />New PropertyPool</button></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
      <Panel title="Create PropertyPool automatically" eyebrow="Collective site visit">
        <div className="space-y-3">
          {properties.map((property) => <button key={property.id} onClick={() => setSelected(property)} className={clsx("w-full rounded-xl border p-4 text-left", selected?.id === property.id ? "border-gold/45 bg-gold/10" : "border-ink/12 bg-ivory")}>
      ...
      <Panel title="Auto-generated event plan" eyebrow="PropertyPool agent">
        {selected ? <div className="space-y-4">
          ...
          <button onClick={onCreate} className="inline-flex items-center gap-2 rounded-xl bg-gold px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1d4ed8]"><CalendarClock size={16} />{busy ? "Creating..." : "Create PropertyPool"}</button>
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1fr_0.8fr]">
      ...
      <Panel title="QR check-in and route" eyebrow="PropertyPool operations">
        <div className="mb-4 grid h-36 w-36 place-items-center rounded-xl border border-ink/12 bg-sand"><QrCode size={84} className="text-ink" /></div>
    ...
    <div className="grid gap-5 xl:grid-cols-[380px_1fr]">
      <Panel title="Add buyer" eyebrow="Lead qualification agent">
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <button className="inline-flex items-center gap-2 rounded-xl bg-ink px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1f2937]"><Plus size={16} />{busy ? "Adding..." : "Add and qualify"}</button>
        </form>
      </Panel>
...
function AutomationView({ dashboard, onRun, busy }: { dashboard: BrokerDashboard; onRun: (createPool?: boolean) => void; busy: string | null }) {
  return <div className="grid gap-5 xl:grid-cols-[0.85fr_1.15fr]"><Panel title="Broker automation controls" eyebrow="Codex operations engine"><div className="space-y-3"><button onClick={() => onRun(false)} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-ink px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1f2937]"><RefreshCw size={16} />{busy === "growth" ? "Running..." : "Run Broker Growth Automation"}</button><button onClick={() => onRun(true)} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gold px-4 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1d4ed8]"><CalendarClock size={16} />{busy === "propertypool-auto" ? "Planning..." : "Create PropertyPool Automatically"}</button></div></Panel><Panel title="Agent task queue" eyebrow="LangGraph broker swarm"><div className="grid gap-3">{dashboard.next_best_actions.map((task) => <TaskRow key={task.id} title={String(task.output_json?.title || task.task_type)} agent={task.agent_name} priority={task.priority} />)}</div></Panel></div>;
}

function MessagesView({ dashboard }: { dashboard: BrokerDashboard }) {
  const event = dashboard.propertypool_events[0];
  return <Panel title="WhatsApp automation drafts" eyebrow="Approved claims and attribution-aware"><div className="grid gap-4 xl:grid-cols-2">{dashboard.buyers.map((buyer) => <div key={buyer.id} className="rounded-xl border border-ink/12 bg-ivory p-4"><div className="flex items-center justify-between"><p className="font-semibold text-ink">{buyer.full_name}</p><MessageCircle size={18} className="text-gold" /></div><p className="mt-3 text-sm leading-7 text-ink/60">{event?.invite_message || `Hi ${buyer.full_name}, I found verified inventory matching your budget. Should I reserve a visit slot?`}</p><button className="mt-3 inline-flex items-center gap-2 rounded-xl bg-gold px-3 py-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1d4ed8]"><Send size={14} />Draft WhatsApp</button></div>)}</div></Panel>;
}
...
function OpportunityCard({ property, onRequestTieup, onCreatePropertyPool, busy }: { property: BrokerProperty; onRequestTieup: (property: BrokerProperty) => void; onCreatePropertyPool: () => void; busy: string | null }) {
  ...
  return <div className="rounded-xl border border-ink/12 bg-ivory p-4"><div className="flex items-start justify-between gap-4"><div><p className="lx-display text-xl font-light text-ink">{property.title}</p><p className="mt-1 text-sm text-ink/55">{property.locality} · {formatCr(property.price)} · {property.commission_range}</p></div><StatusBadge status={property.tieup_status} /></div><div className="mt-4 grid gap-2 md:grid-cols-3"><MiniMetric label="Buyer demand" value={`${Math.round(property.buyer_demand_score)}`} /><MiniMetric label="Legal risk" value={`${Math.round(property.legal_risk_score)}`} /><MiniMetric label="Market heat" value={`${Math.round(property.market_heat_score)}`} /></div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => onRequestTieup(property)} disabled={locked || requesting} className="inline-flex items-center gap-2 rounded-xl bg-ink px-3 py-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1f2937] disabled:cursor-not-allowed disabled:bg-ink/25 disabled:text-ivory/60"><ShieldCheck size={14} />{tieupActionLabel(property.tieup_status, requesting)}</button><button className="inline-flex items-center gap-2 rounded-xl border border-ink/15 bg-ivory px-3 py-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-ink transition-colors hover:bg-sand"><Send size={14} />Share pitch</button><button onClick={onCreatePropertyPool} className="inline-flex items-center gap-2 rounded-xl bg-gold px-3 py-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1d4ed8]"><CalendarClock size={14} />Create PropertyPool</button></div></div>;
}
...
function BuyerCard({ buyer }: { buyer: BrokerBuyer }) {
  ...
  return <Link href={`/broker/buyers/${buyer.id}`} className="rounded-xl border border-ink/12 bg-ivory p-5 shadow-lx transition hover:-translate-y-1"><div className="flex items-start justify-between gap-3"><div><p className="lx-display text-xl font-light text-ink">{buyer.full_name}</p><p className="mt-1 text-sm text-ink/55">{buyer.phone} · {buyer.communication_channel}</p></div><StatusBadge status={buyer.lead_temperature} /></div><div className="mt-4 grid gap-2 md:grid-cols-2"><MiniMetric label="Budget" value={formatCr(buyer.budget_max)} /><MiniMetric label="Qualification" value={`${buyer.qualification_score}/100`} /></div><p className="mt-3 text-sm text-ink/55">{buyer.preferred_localities.join(", ")} · {buyer.follow_up_status}</p><button onClick={callBuyer} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-gold px-3 py-2 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory transition-colors hover:bg-[#1d4ed8]"><MessageCircle size={14} />{busy ? "Calling..." : "Call buyer with ASTRA Voice Closer"}</button>{callResult ? <p className="mt-3 rounded-xl bg-sand p-3 text-xs font-semibold text-ink/70">{callResult.call_status}: {callResult.reason}</p> : null}</Link>;
}
...
function MetricCard({ label, value, detail, tone }: { label: string; value: string; detail: string; tone: string }) {
  ...
  return <div className="rounded-xl border border-ink/12 bg-ivory p-4"><p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-ink/45">{label}</p><p className={clsx("lx-display mt-3 text-3xl font-light", valueColor)}>{value}</p><p className="mt-2 text-sm text-ink/55">{detail}</p></div>;
}

function MiniMetric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-ink/12 bg-ivory p-3"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-ink/45">{label}</p><p className="lx-display mt-1 text-lg font-light text-ink">{value}</p></div>;
}
...
function TaskRow({ title, agent, priority }: { title: string; agent: string; priority: string }) {
  return <div className="rounded-xl border border-ink/12 bg-ivory p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold text-ink">{title}</p><p className="mt-1 text-sm text-ink/55">{agent}</p></div><span className={clsx("rounded-full px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.16em]", priority === "high" ? "bg-gold/12 text-[#1d4ed8]" : "bg-ink/5 text-ink/55")}>{priority}</span></div></div>;
}

function RouteRow({ label, focus }: { label: string; focus: string }) {
  return <div className="rounded-xl border border-ink/12 bg-ivory p-3"><p className="text-sm font-semibold text-ink">{label}</p><p className="mt-1 text-sm text-ink/55">{focus}</p></div>;
}

function TaskChip({ title, done }: { title: string; done: boolean }) {
  return <div className={clsx("flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold", done ? "border-gold/40 bg-gold/10 text-[#1d4ed8]" : "border-ink/15 bg-sand text-ink/70")}><CheckCircle2 size={16} />{title}</div>;
}

function StatusBadge({ status }: { status: string }) {
  const clean = status.replace(/_/g, " ");
  const tone = status.includes("active") || status.includes("approved") || status.includes("hot") || status.includes("ready") || status.includes("scheduled") ? "border-gold/45 bg-gold/10 text-[#1d4ed8]" : status.includes("review") || status.includes("pending") || status.includes("warm") ? "border-ink/20 bg-ink/5 text-ink/60" : "border-ink/15 text-ink/55";
...
function Legend({ color, label }: { color: string; label: string }) {
  return <div className="flex items-center gap-2 rounded-xl border border-ink/12 bg-ivory px-3 py-2 text-xs font-semibold text-ink/60"><span className={clsx("h-3 w-3 rounded-full", color)} />{label}</div>;
}

function Input({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="block"><span className="mb-1 block text-xs font-semibold uppercase tracking-[0.16em] text-ink/50">{label}</span><input value={value} onChange={(event) => onChange(event.target.value)} className="min-h-11 w-full rounded-xl border border-ink/15 bg-ivory px-3 text-sm outline-none focus:border-gold" /></label>;
}
