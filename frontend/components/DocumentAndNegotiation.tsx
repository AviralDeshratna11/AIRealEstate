  ...
  return (
    <section className="grid gap-5 lg:grid-cols-2">
      <div className="rounded-xl border border-ink/12 bg-ivory p-5 shadow-lx">
        ...
        <label className="mt-4 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-ink/15 bg-ivory px-4 py-8 text-sm font-semibold text-ink/65 hover:border-gold hover:bg-sand">
          <FileUp size={18} />
          Upload document
          <input type="file" accept=".pdf,image/*,.txt" onChange={(e) => upload(e.target.files?.[0])} className="sr-only" />
        </label>
        {docResult !== null && <pre className="mt-4 max-h-80 overflow-auto rounded-xl bg-espresso p-3 text-xs text-goldsoft">{JSON.stringify(docResult, null, 2)}</pre>}
      </div>
      ...
      <div className="rounded-xl border border-ink/12 bg-ivory p-5 shadow-lx">
        ...
        <button onClick={optimize} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-gold px-5 py-3 text-[12px] font-semibold uppercase tracking-[0.16em] text-ivory shadow-lx transition hover:bg-[#1d4ed8]">
          <Scale size={17} />
          Run buyer counter-offer demo
        </button>
        {negotiation !== null && <pre className="mt-4 max-h-80 overflow-auto rounded-xl bg-espresso p-3 text-xs text-goldsoft">{JSON.stringify(negotiation, null, 2)}</pre>}
      </div>
    </section>
  );
}
