...
export function MarketDashboard() {
  ...
  if (!insights) return <div className="rounded-xl border border-ink/12 bg-ivory p-5 shadow-lx font-light text-ink">Loading Mumbai market intelligence...</div>;
  ...
  return (
    <section className="rounded-xl border border-ink/12 bg-ivory p-5 shadow-lx">
      <div className="mb-4 flex items-start justify-between gap-4">
        ...
        <div className="rounded-xl bg-gold px-4 py-2 text-center text-ivory shadow-lx">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {insights.inventory_by_price_bucket.map((row) => (
          <div key={row.cost_range} className="rounded-xl border border-ink/12 bg-ivory p-4">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-5">
        {Object.entries(insights.redevelopment.top_micro_markets).map(([market, count]) => (
          <div key={market} className="rounded-xl bg-espresso text-ivory p-3">
