    ...
    <main className="min-h-screen px-4 py-6 text-ink sm:px-6">
      <div className="mx-auto grid min-h-[calc(100vh-48px)] w-full max-w-6xl items-center gap-8 lg:grid-cols-[1fr_520px]">
        <section className="hidden lg:block">
          <Link href="/" className="inline-flex items-center rounded-md bg-ink px-4 py-2 text-sm font-black text-[#f9fafb]">
            ASTRA Estate
          </Link>
          ...
          <div className="mt-8 grid max-w-xl grid-cols-3 gap-3">
            {["Role gated", "Supabase ready", "Demo mode"].map((item) => (
              <div key={item} className="rounded-lg border border-ink/15 bg-[#f9fafb]/75 p-4 text-sm font-black">
                {item}
              </div>
            ))}
          </div>
        </section>
        <section className="rounded-lg border border-ink/15 bg-[#f9fafb]/94 p-5 shadow-soft backdrop-blur sm:p-7">
          <Link href="/" className="mb-7 inline-flex items-center rounded-md bg-ink px-3 py-2 text-xs font-black uppercase tracking-[0.16em] text-[#f9fafb] lg:hidden">
            ASTRA
          </Link>
